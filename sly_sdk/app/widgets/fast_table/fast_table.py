...
from sly_sdk.app.widgets.widget import JinjaWidgets, Widget
...
class FastTable(Widget):
    ...
    :type project_meta: dict, optional
    ...
    @project_meta.setter
    def project_meta(self, meta: Dict) -> None:
    ...
    def _unpack_project_meta(self, project_meta: Dict) -> dict:
        """
        Apply sorting to received data
        ...
        if project_meta is not None:
            if not isinstance(project_meta, dict):
                raise TypeError(
                    f"Project meta possible types: dict. Instead got {type(project_meta)}"
                )
        return project_meta
