...
from sly_sdk.webpy.app import MainServer, StateJson
...
class RadioTabs(Widget):
    ...
    def __init__(
        ...
        if len(titles) != len(contents):
            raise ValueError("titles length must be equal to contents length in RadioTabs widget.")
        if len(titles) > 10:
        ...
        for title, widget, description in zip(titles, contents, descriptions):
            self._items.append(
                RadioTabs.RadioTabPane(title=title, content=widget, subtitle=description)
            )
